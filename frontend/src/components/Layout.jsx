...
export default function Layout({ children }) {
  const { session, running, startSession, stopSession, latest, alerts } = useSession();
  ...
  return (
    <div className="min-h-screen text-white" style={{ background: 'var(--vx-bg)' }}>
        ...
        <div className="flex items-center justify-between px-4 md:px-6 h-14">
          <div className="flex items-center gap-3">
            <button
              ...
              className="md:hidden p-1.5 rounded border"
      ...
      <div className="flex">
        {/* Sidebar */}
        <aside
          className={`${open ? 'block' : 'hidden'} md:block w-60 shrink-0 border-r min-h-[calc(100vh-3.5rem)] sticky top-14`}
          style={{ borderColor: 'var(--vx-border)', background: 'rgba(22,26,34,0.4)' }}
        >
